...
Note that the data merge script above could probably use a revamping from someone who's more tidyverse-savvy than I was in 2020. Also, there are issues with the generation of random bins (rbins) in that script that render them unusable for analysis. So if you want to look at a finer temporal resolution than a coverage bin, you will definitely need to go back and update the EMA merge script.
...
I’ve been doing some reading on Tom’s and others’ work, and it seems that no one really does any substantial quality control checking of geolocation data (or at least they don’t report doing it). Alessandretti in some work reports that errors didn’t exceed X number of meters, but it is unclear how this error was calculated. Kirchner and colleagues sometimes report geographical extent filters (e.g., US or DC polygon) and radius of gyration filters, but no filters to check for simultaneously measured tracks or implausible speeds. So I’ll go with my own a priori standards. If Tom thinks they should be changed, I can always do that later.
...
## Notes to Future Self and/or Analysts
...
Load the final workspace saved in the preceding code block.

For an analysis template and a bunch of old code that I used to analyze previous iterations of the pilot data, see ~/Box Sync/GeoScanning_Data_822815/Data/Scripts/Analysis_Template_6.17.21.Rmd
